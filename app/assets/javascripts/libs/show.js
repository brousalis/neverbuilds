nw.show = function() {
  var init = function(feats, powers) {  
    ...
    $.each(feats, function(k,v) {
      ...
      if(v > 0) button.addClass('on');
      value.html(v + '/');
    });    

    // powers
    $.each(powers, function(k,v) {
      var button = $('[rel='+k+']'),
          rank   = button.find('input'),
          val    = parseInt(v, 10);
      rank.val(val);
      switch(val) {
        case 1:
          button.find('.rank:lt(1)').addClass('on');
          $('.details .desc').addClass('on');
          break; 
        case 2:
          button.find('.rank:lt(2)').addClass('on');
          $('.details .desc, .details .rank_2').addClass('on');
          break;
        case 3:
          $('.details .desc, .details .rank_2, .details .rank_3').addClass('on');
          button.find('.rank:lt(3)').addClass('on');
          break;
      } 
      if(val > 0) button.addClass('enabled');
    });     
